...
events <- 317L; non_events <- n_final - events; prevalence <- events / n_final
...
latex_rhs <- function(model) {
  ...
  groups <- split(pieces, ceiling(seq_along(pieces)/5))
...
- Dez pontos percentuais adicionais de correção transversal pelo colete corresponderam a `r fmt(linear_10pp)`° em `delta` e OR `r fmt(or_10pp, 2)` para melhora, mantidos os demais preditores. OR não é risco relativo e associação prognóstica não é efeito causal.
...
Dez pontos percentuais adicionais de correção pelo colete correspondem a `r fmt(linear_10pp)`° em `delta`, mantidos os demais preditores. O sinal favorável não prova que aumentar a correção cause melhor desfecho nem garante manutenção.
...
A amplitude bootstrap mediana foi `r fmt(median(boot_stability$instability_index_95[boot_stability$model=="linear"]))`° no linear e `r fmt_pct(median(boot_stability$instability_index_95[boot_stability$model=="logistic"]),1)` na probabilidade. É instabilidade entre modelos reamostrados, não intervalo de predição.
...
Rascunho dos itens oficiais, com localização/evidência/pendência. É checklist de relato, não avaliação de qualidade [@collins2024_tripod_ai].
...
Separa qualidade do desenvolvimento, risco de viés da avaliação interna e aplicabilidade. Não é avaliação independente nem julgamento formal definitivo [@moons2025_probast_ai].
