...
Este é o relato completo da análise prognóstica: apresenta população, definições, especificação, coeficientes, avaliação interna e sensibilidades sem exigir leitura de `analisys.qmd`. As decisões tomadas antes do congelamento são descritas aqui porque fazem parte da história metodológica e limitam a interpretação; não são um pré-requisito de leitura.
...
O HTML incorpora figuras e tabelas e pode ser lido isoladamente. Reproduzir os cálculos requer a planilha, os scripts e o pacote de resultados identificado no apêndice; não requer executar ou ler o documento histórico. As lacunas de coleta e ética ainda precisam ser preenchidas pelos investigadores, mesmo em um relato autônomo.
...
A escolha de dez preditores não é justificada por um resultado prévio que o leitor precise consultar: corresponde à lista histórica aqui explicitada, congelada após exploração para esta avaliação. Não existe justificativa prospectiva documentada capaz de transformar esse processo em escolha a priori. Cobb basal foi preservado fora do principal por essa decisão procedimental e estudado explicitamente como sensibilidade.
...
- Desempenho CART e estabilidade bootstrap foram empacotados em `results/prognostico/revisao/aggregated/` como cópias fiéis dos agregados auditados, com origem e SHA-256 em `standalone_inputs_manifest.csv`. O QMD não lê `analisys.qmd` nem agrega resultados a partir dele.
